...
- **login 与 signup 两屏都去掉了 `FARM FROM HERE` 品牌字**（owner 要求）。品牌字只留在 splash。
- 两屏留白宽度已统一，见 §2b。
- splash 不受影响，仍是满屏出血。
...
早先的做法是 `contain`，图完整装进上方区域，于是留白宽度取决于每张图自己的宽高比 ——
signup（0.56）比 login（0.75）窄得多，留白宽一倍多，两屏看着不是一套。

现在改成 **所有 onboarding 插画一律按同一宽度绘制**（屏宽的 84%，每侧留白 8%），
**长宽比不变**，按该宽度放大后**底部对齐**，超出上方区域的部分**从顶部裁掉**。

实测（375×812）：

| 屏 | 素材 | 左边距 | 画出宽度 | 顶部裁掉 | 卡片起点 |
|---|---|---|---|---|---|
| login | 1086×1448 | 30pt | 315pt | 2pt | 422 (52%) |
| signup | 941×1672 | 30pt | 315pt | 137pt | 422 (52%) |

两屏留白完全一致。裁切从顶部走，所以**小狗和小路都留在画面里**——signup 那张的狗现在比之前
大得多、也更显眼。素材不需要重导了。
...
- `scenes` 里**记录了每张图的原始像素尺寸**。`Image.resolveAssetSource` 在 react-native-web 上
  ...
  **换图必须同时改那里的数字**——它决定图按统一宽度放大后有多高，进而决定裁掉多少。
- 插画的留白由 `scrim.artSideInset`（0.08）统一控制，**不要在页面里单独调**，否则两屏又会分家。
- 裁切靠 `justifyContent: 'flex-end'` 底部对齐实现。改成居中或顶部对齐，小狗就会被裁掉。
