...
## 2026-08-19 17:43 — Step 2A:农民申请 → 审核 → 批准(农民后台第一块真东西)

- **在这之前农民后台整个是占位**。这轮把 `apply` 那块换成真的:申请表 → 审核 → 批准建农场。
  `post` / 建地块 / 加认养物 / 编辑农场资料 **仍然是占位**,归 2B/2C。
- **新迁移 `20260819000200_farm_applications.sql`**:三张新表(申请、私密材料、申请照片)、
  两个新 bucket、批准触发器、以及三张表的 RLS。
- **批准是一条路、两种时机**:个体户提交即自动批准,正式农场进 `pending` 等你在 Supabase 后台
  手动翻状态。触发器统一做三件事:建 `farms`、写 `farm_members`(owner)、把申请照片复制进
  `farm_media`。**幂等**靠申请行上的 `created_farm_id` 闩:再翻一次状态什么都不会发生,不会出现
  第二个农场、第二条成员、重复照片。
- **为什么"提交"是一个 RPC 而不是直接插入**:个体户是"提交那一刻"就批准的,而批准会复制照片——
  要是插入时就批准,相册会是空的,反而卡住它本该打开的那道门。所以顺序被钉死:建行 → 传文件 → 提交。
- **解锁门槛现在是两半**:有 `farm_members` **且** `farm_media ≥ 1`。批准时相册被自动播种,
  所以个体户提交完当场两半都满足。批准后会自动重读门槛,不用重启 app。
- **防滥用是数据库在管**:一人一农场、同时只能有一份在审申请(部分唯一索引 + 触发器);被拒的可以
  改了再交,不算新的一份。UI 只是提前问一句好把理由说清楚。
- **⚠️ 动了一条 Step-1 约束**:`farms.latitude/longitude` 改成可空。原来是 `not null`,等于默认
  每个农场都有一个精确的点——但"城市级就够"是已经拍板的规则,而且没有 API 就没法把城市变成坐标。
  新加了 CHECK 保住真正的不变量:农场要么有坐标、要么有写下来的地址。地图页改成没有点就不显示距离、
  不给"用 Google 地图打开"。
- **⚠️ 写的时候堵了一个洞**:插入策略原本只校验"是不是你自己的",所以客户端可以插一行
  `status='approved'`。它不会真的建出农场(批准触发器只挂在 UPDATE 上)——正因如此才更难受:
  界面会给你看"已通过",而农场并不存在。现在插入一律被强制回 `pending`,审核字段清空。
- **⚠️ 迁移一次都没跑过**。这台 Mac 没有 Postgres / psql / supabase CLI / Docker,和之前每一轮
  schema 一样。括号和函数体是平衡的,但第一次 `supabase db push` 才是真正的第一次测试,预计要修一轮。
- 你的三个选择都已落地:**Mapbox**(已接,token 可空——不填就手打城市+州,这本来就是完整答案)、
  **装了 `expo-document-picker`**(正式农场能传真的 PDF 地契)、**分级措辞保持** "Community grower" /
  "Verified farm"。
- **保留的真金白银表按 §9 查过了**:`payments` / `donations` / `rewards` / `redemptions` 的外键都指向
  `profiles` / `farms` / `adoptables` / `seeds_ledger`,都正常;这轮一张都没碰、没启用、没重建。
- 详见 `updates/2026-08-19_CC_step2a-farmer-application.md`。
